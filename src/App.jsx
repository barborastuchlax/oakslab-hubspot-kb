...
- MQL: Talk to Sales form submission, direct meeting booking with Josh, or Clutch direct contact. Talk to Sales and meeting bookings assigned automatically; Clutch leads entered manually by Josh.
...
There are two separate tiers. They are scored independently and may not always match.

### Company Tier (ICP fit)
Set on the Company record. Based on firmographics — does this company match our ideal customer profile? Set on import (outbound) or after first qualification (inbound).

- Tier 1 — Ideal fit: ALL must be true: Series A–D, US-based engineering team with identifiable CTO/CPO, 10–50 engineers, confirmed or strongly implied openness to external engineering partners.
- Tier 2 — Strong fit, one unknown: Most but not all Tier 1 boxes ticked: Series A–D, US-based engineering team with identifiable CTO/CPO, engineering team size unclear or slightly outside sweet spot, openness to external partners unconfirmed.
...
Contact-level Lead Quality Tier is deprecated — no longer used.
...
### Clutch leads note
Clutch leads are NOT covered by any workflow. There is no HubSpot form submission — leads arrive as email notifications from Clutch to Josh. Josh manually creates the contact in HubSpot and sets Lifecycle Stage = MQL. Channel = Clutch. These follow the inbound track.
...
## HOW TO: HANDLE A CLUTCH LEAD (Josh)

Clutch is a B2B marketplace where potential clients can contact OAK'S LAB directly. These are inbound leads — the buyer reaches out to us. Josh manages all Clutch leads.

When a new Clutch lead comes in (email from Clutch Team to Josh):
1. Open the email — note Company Name, Lead Name, Service Requested, Website, and Objectives.
2. Search HubSpot for the company. If it doesn't exist, create it: Company Name, Industry, Country/Region, Website.
3. Search for the contact. If they don't exist, create the contact FROM the company record.
4. Fill contact fields: First Name, Last Name, Job Title (if available), Lead Source Category = Inbound, Channel = Clutch, Contact Owner = Josh Urban.
5. Set Lifecycle Stage to MQL — Clutch leads are direct project inquiries, so they skip Lead and enter as MQL (same as Talk to Sales).
6. Review the lead — does the company fit OAK'S LAB ICP? If not a fit: add a note with reasoning. No deal.
7. If worth pursuing: reply via Clutch Dashboard, schedule a call, then update to SQL once the call is confirmed.
8. After call — fit confirmed: update to Opportunity, create deal (Channel = Inbound, Deal Source = Clutch). Not a fit: add a note.

Note: Clutch leads do NOT trigger any HubSpot workflow — there is no form submission. Josh must manually create the contact and set all fields. The Clutch notification email is the only trigger.
